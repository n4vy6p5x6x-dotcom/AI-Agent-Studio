...
| 工作流 | 可视化拖拽编排 Agent / RAG / HTTP / 条件 / 并行网关 / Tool |
...
工作流是由 **节点（Node）** 和 **连线（Edge）** 组成的有向图，支持线性串联、**并行网关**、**条件分支** 与多 Agent 协同，适用于工业计划、排程、质量追溯等复杂场景。
...
| start | 开始节点 |
| end | 结束节点 |
| agent | 调用指定 AI Agent（8 种工业模板） |
| rag | 行业知识库检索 |
| parallel | **并行网关** — 同时触发多个分支 Agent |
| merge | **合并网关** — 汇总并行分支结果 |
| http | 发送 HTTP 请求（如 MES 状态） |
| condition | 条件分支（通过/否、受控/失控等） |
| tool | 调用 Tool（SMED、SPC 等） |

### 6.3 预置工作流（Seed 4 条）

| 工作流 | ID | 设计要点 |
|--------|-----|----------|
| 无人机协同生产计划 | demo-workflow-001 | 双路并行(MPS+MRP) → 布局/排程 → 条件分支 → 仿真/调度 → 决策 |
| SMT 智能排程与换线优化 | demo-workflow-002 | RAG+MES → 排程/质量并行 → SMED → 急单分支 |
| 新能源质量追溯闭环 | demo-workflow-003 | 追溯/数据并行 → SPC → 失控分支 → 布局/决策并行 |
| 多 Agent 全局协同决策 | demo-workflow-004 | 合同网 → 三路竞标并行 → 冲突消解 → 仿真 → 报告 |

### 6.4 操作步骤

#### 选择工作流

顶部下拉框选择上述 4 条预置工作流，或自行创建。

#### 添加节点

点击工具栏：`agent` / `rag` / `parallel` / `merge` / `http` / `condition` / `tool`
...
```
开始 → RAG → [并行: 计划员 + 数据员] → 合并 → RAG → [并行: 设计师 + 排程员] → 合并
  → 条件判断 → (通过→仿真员 / 否→调度员) → 合并 → 协同决策员 → 结束
```
...
1. 优先选用 4 条预置工业工作流作为模板  
2. 复杂场景使用 **parallel + merge** 实现多 Agent 并行  
3. 在 RAG 节点后接 Agent 节点，提升回答质量  
4. 条件节点可配置急单、SPC 失控、资源冲突等分支  
5. 每次修改后保存并运行，查看右侧执行日志
